          ...
          tagValueParsers: {
            sassVariable: function(tagContent, fileContents) {
              ...
              if(match = regex.exec(fileContents)) {
                //we don't want the !default in the value, we store that as a seperate property
                var value = match[1].replace(/\s\!default/, '');
                var isDefault = match[1].match(/\s\!default/) != null ? true : false;
